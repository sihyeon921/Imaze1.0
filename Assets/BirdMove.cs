...
public class BirdMove : MonoBehaviour {
    ...
    public Vector3 target; //추적할 오브젝트
    Vector3 posTarget; //추적할 목표 좌표
    public Rigidbody2D rig;
    public Animator anim;
    public Transform A;
    public float distance;
    public float rotateVt = 5f; //z축 회전율
    ...
    public float homingAngleA = 0.1f; //호밍 각도 변화 가속도
    ...
    void Update()
    {
        distance = Vector3.Distance(A.position, this.transform.position);
        if (fm.arrive)
        {
            anim.SetBool("fly", true);
            transform.Rotate(0, 0, Time.deltaTime * rotateVt / 50); //시간에 따라 물체가 회전
        }
        if(distance > 25f)
            Destroy(gameObject);
    }
    ...
    void FixedUpdate()
    {
        if (fm.arrive)
        {
            if (left) //왼쪽 회전 이동시
            {
                posTarget = target; //목표 오브젝트 좌표 받아옴
